...
function convertFromPages(pages) {    //Pages starts from 1 to 604
...
function convertFromSurahs(surahs) {  //Surahs starts from 0 to 113
    let correspondingLines = generateLines(false);

    for (let i = 0, length = surahs.length; i < length; i++) {
        let startPageOfCurrent = startPageOfSurahs[surahs[i]];
        let startLineOfCurrent = surahs[i] !== 0 ? endLineOfSurahs[surahs[i] - 1] !== 15 ? endLineOfSurahs[surahs[i] - 1] + 1 : 1 : 1;
        let finishLineOfCurrent = endLineOfSurahs[surahs[i]];
        let finishPageOfCurrent = surahs[i] !== 113 ? startPageOfSurahs[surahs[i] + 1] - (finishLineOfCurrent === 15 ? 1 : 0) : 604;

        for (let j = startPageOfCurrent; j <= finishPageOfCurrent; j++) {
            if (startPageOfCurrent === finishPageOfCurrent) {
                correspondingLines[startPageOfCurrent - 1] = dec2bin(bin2dec(correspondingLines[startPageOfCurrent - 1]) | bin2dec(
                ...
                if (j === startPageOfCurrent) {
                    correspondingLines[j - 1] = dec2bin(bin2dec(correspondingLines[j - 1]) | bin2dec(
                        "0".repeat(startLineOfCurrent - 1)
                        + "1".repeat(15 - startLineOfCurrent + 1)
                    ));
...
function convertFromJuzs(juzs) {            //Juz's start from 0 to 29
    juzs = juzs.slice();
    juzs.sort((a, b) => a - b);
...
function convertFromLines(lines) {  //donner en argument tableau des lignes
    ...
    for (let i = 0, length = startPageOfSurahs.length; i < length; i++) {
        ...
        for (let j = startPageOfCurrent; j <= finishPageOfCurrent; j++) {
            if (startPageOfCurrent === finishPageOfCurrent) {
                if ((bin2dec(lines[j - 1]) | bin2dec("1".repeat(startLineOfCurrent - 1)
                    + "0".repeat(finishLineOfCurrent - startLineOfCurrent + 1)
                    + "1".repeat(15 - (finishLineOfCurrent)))) !== FULL_PAGE) {
                    surrahIsIncluded = false;
                }

            } else {
                if (j === startPageOfCurrent &&
                    (bin2dec(lines[j - 1]) | bin2dec("1".repeat(startLineOfCurrent - 1) + "0".repeat(15 - startLineOfCurrent + 1))) !== FULL_PAGE) {
                    surrahIsIncluded = false;
                } else if (j !== startPageOfCurrent && j !== finishPageOfCurrent && bin2dec(lines[j - 1]) !== FULL_PAGE) {
                    surrahIsIncluded = false;
                } else if (j === finishPageOfCurrent &&
                    (bin2dec(lines[j - 1]) | bin2dec("0".repeat(finishLineOfCurrent) + "1".repeat(15 - finishLineOfCurrent))) !== FULL_PAGE) {
                    surrahIsIncluded = false;
                }
            }
        }
...
function getSurahsContainingTheseLines(lines) {  //toutes les sourates contenant au moin une page incluse seront incluses
    const FULL_PAGE = 0b111111111111111;
    let correspondingSurahs = [];
...
export { surahInfo, convertFromJuzs, convertFromPages, convertFromSurahs, convertFromLines, getSurahsContainingTheseLines };