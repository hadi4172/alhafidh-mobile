...
import { surahInfo, getSurahsContainingTheseLines } from "../../Data/quranStats";
...
function OrderSelectScreen(props) {
    ...
    if (storeVariableNameChecked === "familiar") {
        ...
        selectCheckedSurahs = selectCheckedSurahs.filter(s => selectOrderedFamiliarSurahs[s - 1] === 0);
    }
    ...
    return (
        <View style={[styles.showBorder, { flex: 1 }]}>
            <View style={[styles.showBorder, { flex: 0 }]}>
                <CustomHeader
                    title={screen}
                    innerText={"Sélectionnez l’ordre de mémorisation des parties familières"}
